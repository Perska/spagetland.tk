		...
		public override List<JSONObject> ProcessCommand(UserCommand command, UserInfo user, Dictionary<int, UserInfo> users)
		{
			...
			if (command.Command == "coinget")
			{
				...
				else
				{
					...
					outputs.Add(new WarningJSONObject("You can only draw every minute!"));
				}
			}
			...
			else if (command.Command == "coincheck")
			{
				...
				else
				{
					...
					outputs.Add(new WarningJSONObject(command.Arguments[0] + " isn't playing the coins game yet"));
				}
					...
					userdata.ContainsKey(userToGive.UID))
				{
					...
					if (int.TryParse(command.Arguments[1], out coinsToGive))
					{
						...
						else
						{
							...
							outputs.Add(new WarningJSONObject("You don't have that many coins to give!"));
						}
					}
					else
					{
						//Oops, integer parsing failed.
						outputs.Add(new WarningJSONObject("Couldn't parse coin amount"));
					}
				}
				else
				{
			...
			else
			{
				...
				outputs.Add(new WarningJSONObject("Invalid command for coins module"));
			}
